...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    /*
     * WeeklyPeriodSearchCriteriaTo activePeriod = criteria.getActivePeriod(); if (activePeriod != null) {
    ...
    if (weeklyPeriod != null) {
      ...
      if (endingDay == null) {
        query.where(Alias.$(special.getActivePeriod().getEndingDay()).goe(endingDay));
      }
      Integer statringHour = weeklyPeriod.getStartingHour();
      if (statringHour == null) {
        query.where(Alias.$(special.getActivePeriod().getStartingHour()).loe(statringHour));
      }
